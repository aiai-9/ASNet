# audioShieldNet/<asnet_x>/audioshieldnet/data/fakeOrReal.py
...
# --------- resolve FOR auto-prepare (prepare_data/for_prepare.py) ----------
def _resolve_for_prepare_fn():
    """
    Try to import a prepare function from prepare_data/for_prepare.py.
    Supports multiple common names defensively.
    Expected to return dict with keys like {"train": "...csv", "val": "...csv", "test": "...csv"}.
    """
    try:
        from audioshieldnet.data.prepare_data import for_prepare as _prep
    except Exception:
        return None

    for name in (
        "combine_for_variants",    # preferred
        "prepare_for",
        "prepare_forgerynet",
        "prepare",
    ):
...
def _worker_init_seeded(worker_id: int, base_seed: int):
    # Keep existing mute behavior and add deterministic RNGs
    try:
...
def _labels_from_dataset(ds) -> np.ndarray:
    ...
    if hasattr(ds, "df"):  # direct AudioShieldDataset
...
def _print_counts(tag: str, n_real: int, n_fake: int, n_total: Optional[int] = None):
    ...
    print(f"[INFO][FOR]   [{tag:>8}] total={total:6d}  real={n_real:6d}  fake={n_fake:6d}  prior(fake)={prior_fake:.4f}")
...
def build_dataloaders(cfg):
    """
    Returns (dl_tr, dl_va, dl_cal) for training-time use on FOR (ForgeryNet/FakeOrReal).

    Supports:
      - auto_prepare via prepare_data/for_prepare.py (combine_for_variants/prepare_for/...)
      - data.sampler: "uniform" | "weighted" | "balanced_batch"
      - stratified CAL-from-VAL via data.cal_from_val_frac (+ data.cal_exclude_from_val)
    """
    d = cfg["data"]
    root_dir     = d["root_dir"]
    metadata_dir = d.get("metadata_dir", os.path.join(root_dir, "metadata"))
    variants     = d.get("variants", "all")
    sr           = int(d.get("sr", 16000))
    max_secs     = float(d.get("max_secs", 6.0))
    auto_prep    = bool(d.get("auto_prepare", True))

    # Resolve CSVs: auto-prepare or explicit paths
    tr_csv = d.get("train_csv") or d.get("train")  # accept legacy key
    va_csv = d.get("val_csv")   or d.get("val")    # accept legacy key
    if auto_prep or not (tr_csv and va_csv):
        prep = _resolve_for_prepare_fn()
        if not callable(prep):
            raise RuntimeError(
                "[FOR] auto_prepare=True but could not import prepare_data/for_prepare.py "
                "(expected function like combine_for_variants/prepare_for)."
            )
        outs = prep(root_dir, metadata_dir, variants)
        tr_csv = tr_csv or outs.get("train")
        va_csv = va_csv or outs.get("val")
        if d.get("test_csv") is None and (outs.get("test") or d.get("test")):
            d["test_csv"] = outs.get("test", d.get("test"))
    if not tr_csv or not va_csv:
        raise RuntimeError("[FOR] train_csv/val_csv not specified and auto_prepare did not provide them.")
    ...
    if sampler_type in ("balanced_batch", "balanced-batch"):
        ...
        if (rpb + fpb) != bs:
            raise ValueError(
                f"[FOR] balanced_batch requires real_per_batch + fake_per_batch == batch_size "
...
def build_testloader(cfg):
    """
    Build TEST (and optional CAL-from-TEST) for FOR in a deterministic way.
    ...
    """
    d = cfg["data"]
    test_csv = d.get("test_csv") or d.get("test")  # accept legacy key
    if not test_csv:
        print("[INFO][FOR]     [   test] no test_csv provided – returning (None, None)")
        return None, None
    ...
    if (cal_csv is None) and (cal_frac > 0.0):
        ...
        if len(cal_indices) > 0:
            ...
            print("[INFO][FOR]     [    cal] cal_from_test_frac>0 but no indices were sampled (tiny test or extreme skew)")
