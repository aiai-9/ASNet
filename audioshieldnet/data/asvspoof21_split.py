# audioShieldNet/asnet_6/audioshieldnet/data/asvspoof21_split.py
...
def _labels_from_dataset(ds) -> np.ndarray:
    ...
    if isinstance(ds, Subset):
        ...
        labs = _labels_from_dataset(base)  # nested subsets
...
def _print_counts(tag: str, n_real: int, n_fake: int, n_total: Optional[int] = None):
    ...
    print(f"[INFO][ASV21][{tag:>8}] total={total:6d}  real={n_real:6d}  fake={n_fake:6d}  prior(fake)={prior_fake:.4f}")
...
class BalancedBatchSampler(Sampler[int]):
    """
    Yields indices in groups that form balanced batches (real=fake each batch).
    Assumes binary labels {0=real, 1=fake}. Epoch length = n_batches * (rpb + fpb).

    IMPORTANT:
      - Set batch_size == real_per_batch + fake_per_batch in your config.
      - If steps_per_epoch is provided, we use that as n_batches; otherwise we
        fill floor(N / batch_size) batches per epoch (N = dataset size).
    """
    def __init__(
        self,
        labels: np.ndarray,
        ...
        *,
        ...
        # default n_batches if not given: use as many full balanced batches as fit
    ...
    def __iter__(self):
        # Shuffle source pools every epoch
        ...
        for _ in range(self.n_batches):
            # refresh pointers (cyclic) if exhausted
            ...
            # Yield indices one-by-one; DataLoader will pack them into batches.
            for i in batch_idx:
                yield int(i)
                emitted += 1
...
def _make_loader(dataset, cfg, *, sampler=None, shuffle=False, tag="train", generator: Optional[torch.Generator] = None):
    ...
    return DataLoader(
        ...
        prefetch_factor=pf,
        worker_init_fn=_wif,
        generator=generator,  # critical for determinism & reproducible sampling
    )
...
def build_dataloaders(cfg):
    """
    Returns (dl_tr, dl_va, dl_cal) for training-time use on ASVspoof2021.
      - TRAIN: uniform shuffle, WeightedRandomSampler, or BalancedBatchSampler (cfg.data.sampler).
      - VAL:   sequential loader.
    ...
    if sampler_type in ("balanced_batch", "balanced-batch"):
        ...
        if (rpb + fpb) != bs:
            raise ValueError(
                f"[ASV21] balanced_batch requires real_per_batch + fake_per_batch == batch_size "
                f"({rpb} + {fpb} != {bs}). Fix your YAML."
            )
        # decide number of batches per epoch
        ...
        # If steps_per_epoch is set, respect it to stabilize epoch length.
    ...
    # Loaders (all get the same generator for reproducibility)
    ...
    # Store for downstream usage (bias init, logs, etc.)
...
def build_testloader(cfg):
    """
    Build TEST (and optional CAL-from-TEST) for ASVspoof2021 in a deterministic way.
    Controlled by:
      data.test_csv
    ...
    if not test_csv:
        print("[INFO][ASV21][   test] no test_csv provided – returning (None, None)")
        return None, None
    ...
    if (cal_csv is None) and (cal_frac > 0.0):
        ...
        if len(cal_indices) > 0:
            ...
            print("[INFO][ASV21][    cal] cal_from_test_frac>0 but no indices were sampled (tiny test or extreme skew)")
