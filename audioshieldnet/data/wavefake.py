# audioShieldNet/<asnet_x>/audioshieldnet/data/wavefake.py
...
# ---------- resolve prepare_data/wavefake_prepare.py ----------
def _resolve_wavefake_prepare_fn():
    """
    Try to import a WaveFake CSV builder from prepare_data/wavefake_prepare.py.
    Returns a callable like make_wavefake_csvs(...) or None.
    """
    try:
        from audioshieldnet.data.prepare_data import wavefake_prepare as _prep
    except Exception:
        return None

    for name in ("make_wavefake_csvs", "prepare_wavefake", "prepare"):
...
def _print_counts(tag: str, n_real: int, n_fake: int, n_total: Optional[int] = None):
    ...
    print(f"[INFO][WaveFake][{tag:>8}] total={total:6d}  real={n_real:6d}  fake={n_fake:6d}  prior(fake)={prior_fake:.4f}")
...
# -------------------------
# Balanced batch sampler (real=fake per batch)
# -------------------------
...
def build_dataloaders(cfg):
    """
    Returns (dl_tr, dl_va, dl_cal) for training-time use on WaveFake.

    cfg.data can specify either explicit CSVs or enable auto_prepare via
    prepare_data/wavefake_prepare.py to build CSVs from a WaveFake root.

    Supports:
      data.sampler: "uniform" | "weighted" | "balanced_batch"
      data.balanced_batch: {real_per_batch, fake_per_batch}  (if balanced_batch)
      data.cal_from_val_frac, data.cal_exclude_from_val (optional)
    """
    d = cfg["data"]
    root_dir = d["root_dir"]
    sr       = int(d.get("sr", 16000))
    max_secs = float(d.get("max_secs", 6.0))
    auto     = bool(d.get("auto_prepare", False))

    # Resolve CSVs: auto-prepare or explicit paths
    tr_csv = d.get("train_csv")
    va_csv = d.get("val_csv")
    if auto or not (tr_csv and va_csv):
        prep = _resolve_wavefake_prepare_fn()
        if not callable(prep):
            raise RuntimeError(
                "[WaveFake] auto_prepare=True but could not import prepare_data/wavefake_prepare.py "
                "(expected make_wavefake_csvs/prepare_wavefake/prepare)."
            )
        outs = prep(
            root_dir,
            fake_roots=d.get("fake_roots", None),
            real_roots=d.get("real_roots", None),
            split=d.get("split", {"train": 0.8, "val": 0.1, "test": 0.1}),
            seed=int(cfg.get("train", {}).get("seed", 42)),
            out_dir=d.get("out_dir", None),
        )
        tr_csv = tr_csv or outs.get("train_csv") or outs.get("train")
        va_csv = va_csv or outs.get("val_csv")   or outs.get("val")
        if d.get("test_csv") is None and (outs.get("test_csv") or outs.get("test")):
            d["test_csv"] = outs.get("test_csv", outs.get("test"))
    if not tr_csv or not va_csv:
        raise RuntimeError("[WaveFake] train_csv/val_csv not specified and auto_prepare did not provide them.")
    ...
    if sampler_type in ("balanced_batch", "balanced-batch"):
        ...
        if (rpb + fpb) != bs:
            raise ValueError(
                f"[WaveFake] balanced_batch requires real_per_batch + fake_per_batch == batch_size "
        ...
        sampler_tr = BalancedBatchSampler(
            labels_tr, rpb, fpb, seed=seed, n_batches=n_batches
        )
        shuffle_tr = False
...
def build_testloader(cfg):
    ...
    Controls (in cfg.data):
      test_csv
      cal_csv (explicit) OR
      cal_from_test_frac (float, 0..1) + cal_exclude_from_test (bool)
    Returns: dl_te, dl_cal (dl_cal may be None)
    """
    d = cfg["data"]
    test_csv = d.get("test_csv", None)
    if not test_csv:
        print("[INFO][WaveFake][   test] no test_csv provided – returning (None, None)")
        return None, None
    ...
    if (cal_csv is None) and (cal_frac > 0.0):
        ...
        if len(cal_indices) > 0:
            ...
            print("[INFO][WaveFake][    cal] cal_from_test_frac>0 but no indices were sampled (tiny test or extreme skew)")
